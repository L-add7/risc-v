...
TOP                     ?= riscv
VCS                     := vcs -full64 -cpp g++-11 -cc gcc-11 -LDFLAGS -Wl,--no-as-needed 
VCS_OPTS                := -notice -line +lint=all,noVCDE,noNS,noSVA-UA -sverilog -timescale=1ns/10ps -debug_acc+pp+f+fn+dmptf -debug_region+cell+encrypt
VCS_VERDI 				:= vcs -full64 -cpp g++-11 -cc gcc-11 -LDFLAGS -Wl,--no-as-needed 
VCS_VERDI_OPTS			:= -sverilog -kdb -lca -debug_access+all -fsdb +define+FSDB 
...
sim/%.fsdb: 
	cd sim && $(VCS_VERDI) $(VCS_VERDI_OPTS) $(RTL) $*.v 

verdi:
	cd sim && ./simv -gui=verdi
...
clean:
	...
	*.vpd \
	verdiLog \
	sim/*.fsdb sim/*rc sim/*.conf sim/simv sim/verdiLog \
	ucli.key
	rm -rf DVEfiles *.key *.vpd xsim.dir xvlog.log xvlog.pb
